...
public class NegamaxSearch {
    ...
    public static SearchResult pickNextMove(Game game) {
        return pickNextMoveWithNegamax(game);
    }

    private static SearchResult pickNextMoveWithNegamax(Game game) {
        return search(game, 4);
    }

    /** Run a single fixed-depth search and return best move + score. */
    public static SearchResult search(Game game, int depth) {
        nodes = 0;
        ...
        return new SearchResult(bestMove, bestScore, nodes, timeMs, nps, pvLine);
    }


    /** Core negamax with alpha-beta. */
    private static int negamax(Game game, int depth, int ply, int alpha, int beta) {
        nodes++;
