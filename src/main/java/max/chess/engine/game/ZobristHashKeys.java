...
public class ZobristHashKeys {
    private static final int CASTLING_HASH_OFFSET = 768;
    ...
    public static long switchWhiteKingSideCastle(long key) {
        return key ^ HASH_KEYS[CASTLING_HASH_OFFSET];
    }

    public static long switchWhiteQueenSideCastle(long key) {
        return key ^ HASH_KEYS[CASTLING_HASH_OFFSET+1];
    }

    public static long switchBlackKingSideCastle(long key) {
        return key ^ HASH_KEYS[CASTLING_HASH_OFFSET+2];
    }

    public static long switchBlackQueenSideCastle(long key) {
        return key ^ HASH_KEYS[CASTLING_HASH_OFFSET+3];
    }

    public static long switchPlayer(long key) {
        return key ^ HASH_KEYS[TURN_HASH_OFFSET];
    }

    public static long changeEnPassant(long key, int previousEnPassantFile, int newEnPassantFile) {
        if(previousEnPassantFile != -1) {
            key ^= HASH_KEYS[EN_PASSANT_HASH_OFFSET+previousEnPassantFile];
        }

        if(newEnPassantFile != -1) {
            key ^= HASH_KEYS[EN_PASSANT_HASH_OFFSET+newEnPassantFile];
        }
    ...
    public static long switchPiecePresence(long key, byte pieceType, int color, int positionIndex) {
        ...
        // << 6 <-> * 64
        int pieceOffset = (kindOfPiece << 6) + positionIndex;

        return key ^ HASH_KEYS[pieceOffset];
    }

    public static long getHashKey(Game gameRepresentation) {
        long pieceHash = 0;
        byte[] squares = gameRepresentation.board().generateSquares();
        // Encoding the pieces
        for(int i = 0 ; i < 64 ; i++) {
        ...
        // Encoding castling
        long castlingHash = 0;
        if(gameRepresentation.whiteCanCastleKingSide) {
            castlingHash = switchWhiteKingSideCastle(castlingHash);
        }
        if(gameRepresentation.whiteCanCastleQueenSide) {
            castlingHash = switchWhiteQueenSideCastle(castlingHash);
        }
        if(gameRepresentation.blackCanCastleKingSide) {
            castlingHash = switchBlackKingSideCastle(castlingHash);
        }
        if(gameRepresentation.blackCanCastleQueenSide) {
            castlingHash = switchBlackQueenSideCastle(castlingHash);
        }
         ...
         */
        long enPassantHash = 0;
        int zobriskEnPassantFile = gameRepresentation.board().enPassantFile;
        if(zobriskEnPassantFile != -1) {
            enPassantHash = changeEnPassant(enPassantHash, -1, zobriskEnPassantFile);
        }

        // Encoding turn
        long turnHash = 0;
        if(ColorUtils.isWhite(gameRepresentation.currentPlayer)) {
            turnHash = switchPlayer(turnHash);
        }

        return pieceHash^castlingHash^enPassantHash^turnHash;
    }

    // 781 entries
    private final static long[] HASH_KEYS = {
